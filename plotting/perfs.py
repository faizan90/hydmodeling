...
#                         dtype=float)
                    ...
                    ens_df_name = (
                        f'ensemble_q_df_cat_{cat}_{calib_valid_lab}_'
                        f'{opt_iter}_kf_{kf}_freq_{freq}.csv')

                    ens_obs_df = ensemble_avg_sims_df.copy()

                    ens_obs_df['obs'] = qobs_arr

                    ens_obs_df.to_csv(
                        os.path.join(qsims_dir, ens_df_name),
                        sep=';',
                        index=False,
                        float_format='%0.3f')
...
def plot_calib_valid_wvcb(
    ...
    for qsim_file in qsim_files:
        ...
        for kf in range(1, kfolds + 1):
            ...
            ens_obs_dfs = {
                lab:pd.read_csv(
                    os.path.join(qsims_dir,
                        f'ensemble_q_df_cat_{cat}_{lab}_'
                        f'{opt_iter}_kf_{kf}_freq_{freq}.csv'),
                    sep=text_sep)
                for lab in labs}

            for perf_lab in perf_labs:
                ...
                for lab in labs:
                    vals = perfs_wvcbs_dfs[lab].loc[
                        best_prm_idxs_df.loc[f'kf_{kf:02d}_idxs'], perf_lab]

                    vals_df.loc[lo_hi_lc_labs, lab] = vals.values

                    if perf_lab not in eff_ftns_dict:
                        continue

                    # location important, size needed later
                    ens_perfs[lab] = {}

                    for i, ens_lab in enumerate(ens_obs_dfs[lab].columns):
                        if ens_lab == 'obs':
                            continue

                        ens_perf = eff_ftns_dict[perf_lab](
                            ens_obs_dfs[lab]['obs'].values,
                            ens_obs_dfs[lab][ens_lab].values,
                            0)

                        ens_perfs[lab][ens_lab] = ens_perf

                        vals_df.loc[ens_lab + ' ens', lab] = ens_perf

                out_txt_name = (
                    f'calib_valid_{perf_lab}_cat_{cat}_'
                    f'kf_{kf:02d}_{opt_iter}.csv')
                ...
                plt.scatter(
                    perfs_wvcbs_dfs[labs[0]].loc[:, perf_lab].values,
                    perfs_wvcbs_dfs[labs[1]].loc[:, perf_lab].values,
                    marker='o',
                    c=perfs_wvcbs_dfs[labs[0]].loc[:, 'wvcb'].values,
                    alpha=0.2)

#                 equal_line = np.linspace(
#                     min(perfs_wvcbs_dfs[labs[0]].loc[:, perf_lab].values.min(),
#                         perfs_wvcbs_dfs[labs[1]].loc[:, perf_lab].values.min()),
#                     max(perfs_wvcbs_dfs[labs[0]].loc[:, perf_lab].values.max(),
#                         perfs_wvcbs_dfs[labs[1]].loc[:, perf_lab].values.max()),
#                     30)

                equal_line = np.linspace(0.0, 1.0, 30)
                ...
                if ens_perfs:
                    for i, ens_lab in enumerate(ens_perfs[labs[0]]):
                        plt.scatter(
                            ens_perfs[labs[0]][ens_lab],
                            ens_perfs[labs[1]][ens_lab],
                            label=ens_lab + ' ens',
                            c=LC_CLRS[i],
                            marker='X',
                            alpha=0.9)

                for i, idx in enumerate(lo_hi_lc_labs):
                    plt.scatter(
                        vals_df.loc[idx, labs[0]],
                        vals_df.loc[idx, labs[1]],
                        label=idx,
                        c=LC_CLRS[i],
                        alpha=0.9)

                cb = plt.colorbar(cmap_mappable)
                cb.set_ticks([0, 1])
                ...
                plt.xlim(equal_line.min(), equal_line.max())
                plt.ylim(equal_line.min(), equal_line.max())

                plt.xlabel(labs[0])
                plt.ylabel(labs[1])

                plt.title(f'{perf_lab} calib/valid movement for cat {cat}')
...
def plot_peak_cntmnt_cts(
    ...
    if save_text_flag:
        ...
        text_ser.to_csv(text_path, sep=text_sep, header=True)
    return
...
def plot_ensemble_mean_sim_sq_diffs(
    ...
    if save_text_flag:
        ...
        text_path = cnvt_fig_path_to_csv_path(out_path)
        text_ser.to_csv(
            text_path, float_format='%0.4f', sep=text_sep, header=True)
    return
