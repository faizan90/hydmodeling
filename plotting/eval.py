...
#             fin_cumm_rho_arrs[:, break_idx] / fin_cumm_rho_arrs[:, -1])
        ...
        plt.axvline(self.break_idx, color='orange')
    ...
    def run_prm_sims(self, *args):
        ...
        if self.opt_iter is None:
            ...
            if n_max_opt_iters < self.opt_iter:
                raise ValueError(
    ...
    def _plot_effs(self):
        ...
        plt.title(f'NS. vs. Pcorr (n={self.sim_perfs_df.shape[0]})')

        plt.scatter(
            self.sim_perfs_df['ns'].values,
            self.sim_perfs_df['pcorr'].values,
            c=self.sim_perfs_df['clrs'].values,
            alpha=0.05)

        plt.scatter(
            self.sim_perfs_df['ns'].values.mean(),
            self.sim_perfs_df['pcorr'].values.mean(),
            c='k',
            alpha=0.5,
            label='Mean')
        ...
        for i in range(self.n_lcs):
            plt.scatter(
                ...
                self.sim_perfs_df.loc[self.lc_idxs[i], 'pcorr'],
                c=self.lc_clrs[i],
                alpha=0.5,
...
#         plt.ylim(-0.1, 1.1)
        ...
        plt.savefig(
            os.path.join(self.ns_vs_pcorr_dir, out_fig_name),
            bbox_inches='tight')

        plt.close()
        return

    def _plot_effs2(self):

        assert self._set_lo_hi_corr_idxs_flag

        plt.figure(figsize=(10, 10))

        plt.title(f'NS. vs. Clrs (n={self.sim_perfs_df.shape[0]})')

        plt.scatter(
            self.sim_perfs_df['ns'].values,
            self.sim_perfs_df['clrs'].values,
            c=self.sim_perfs_df['clrs'].values,
            alpha=0.05)

        plt.scatter(
            self.sim_perfs_df['ns'].values.mean(),
            self.sim_perfs_df['clrs'].values.mean(),
            c='k',
            alpha=0.5,
            label='Mean')
        ...
        for i in range(self.n_lcs):
            plt.scatter(
                self.sim_perfs_df.loc[self.lc_idxs[i], 'ns'],
                self.sim_perfs_df.loc[self.lc_idxs[i], 'clrs'],
                c=self.lc_clrs[i],
                alpha=0.5,
                label=self.lc_labs[i])
...
#         plt.ylim(-0.1, 1.1)
        ...
        plt.savefig(
            os.path.join(self.ns_vs_clrs_dir, out_fig_name),
            bbox_inches='tight')
    ...
    def _get_lo_hi_corr_idxs(self):
        ...
        assert self.sim_perfs_df['ns'].shape[0] >= (n_hi_vals + n_lo_vals)

        acc_flags_ser = ((self.sim_perfs_df['ns'] - mean_ns).abs().rank() <= (
            n_hi_vals + n_lo_vals)).values
        ...
        self.lc_clrs = ['blue', 'green', 'purple', 'orange']
    ...
    def _plot_best_hi_lo_freq_sims(self, out_df, kf):
        ...
        for k, eff_ftn_lab in enumerate(eff_ftn_labs):
            for i in range(self.n_lcs):
                corr_tit_str += f'{self.lc_labs[i]} {eff_ftn_lab}: '
                for j in range(n_resamp_freqs):
                    corr_tit_str += (
                        f'({effs[i][j][0]}: {effs[i][j][k + 1]:7.6f}), ')
        ...
        while lst_idx < self.n_steps:
            ...
            plt.title(title_str, fontsize=6)
    ...
    def _plot_fdc(self, out_df, kf):
        ...
        probs_vals = []
        for i in range(self.n_lcs):
            sim_ser = out_df.loc[:, f'kf_{kf:02d}_sim_{self.lc_idxs[i]:04d}'
                                 ].iloc[self.off_idx:]
